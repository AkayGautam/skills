...
const APP_URL = (process.env.APP_URL || process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000').replace(/\/$/, '');

/**
 * ✅ Helper: normalize stored video_url into a full absolute URL.
 *  - Removes duplicate "uploads/uploads/"
 *  - Converts "upload/" or "public/upload/" to "uploads/"
 *  - Always returns an absolute URL (APP_URL + /uploads/videos/...)
 */
function toAbsoluteVideoUrl(videoUrl) {
  ...
  let cleaned = String(videoUrl).trim();

  // Remove leading slashes
  cleaned = cleaned.replace(/^\/+/, '');

  // Clean known duplicate or wrong folder prefixes
  cleaned = cleaned.replace(/^uploads\/uploads\//, 'uploads/');
  cleaned = cleaned.replace(/^public\/upload\//, 'uploads/');
  cleaned = cleaned.replace(/^upload\//, 'uploads/');
  ...
  // ✅ Final absolute URL
  const absolute = `${APP_URL}/${cleaned}`;
  console.log(`[video.js] Normalized video_url: ${videoUrl} → ${absolute}`);
  return absolute;
}
...
router.get('/videos/:videoId', async (req, res) => {
  try {
    ...
    const video = rows[0];
    video.video_url = toAbsoluteVideoUrl(video.video_url);
...
router.get('/:lessonId/videos', async (req, res) => {
  try {
    ...
    const videos = (rows || []).map((v) => ({
      ...v,
      video_url: toAbsoluteVideoUrl(v.video_url),
    }));
...
router.post('/:lessonId/videos', requireAuth, async (req, res) => {
  try {
    ...
    console.error('POST /api/lessons/:lessonId/videos error', err);
    res.status(500).json({ error: 'Server error' });
  }
});
